...
exports.getClusterIdentifiers = getClusterIdentifiers;
...
function extractClusteredPDBs(mmseqClusterTSVFile) {
    let fileContents = fs_1.default.readFileSync(mmseqClusterTSVFile, 'utf8');
    ...
    ([...uniqueReps]).forEach(rep => {
        let allMembers = fileContents.split('\n').filter(line => line.includes(rep)).map(line => line.split('\t')[1].split('_')[0].toLowerCase());
        clusters.push({
...
function getClusterIdentifiers(pdbId) {
    let clusterIds = [];
