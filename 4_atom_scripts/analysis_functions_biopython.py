...
def place_all_centers(s, 
    ...
    '''
    parser = PDBParser()
    structure = parser.get_structure('PROTEIN', s)
    print(structure)
    prot = [atom.get_coord() for atom in structure.get_atoms() if atom.get_parent().get_resname() != 'HOH']
    out_coords = np.array([])
    sz_all = np.array([])
    out_coords_all={}
    out_coords_all_dens={}
    for r in structure.get_residues():
        if r.get_resname() in list(DICT4A.keys()):
            print(r.get_resname())
...
def place_all_wat_plot(all_coord_info, s,
    ...
    for r in structure.get_residues():
        if r.get_resname() in list(DICT4A.keys()):
            pdb_out = f'{pdb_name}_{r.get_resname()}_{r.get_id()[1]}_{r.get_full_id()[2]}.pdb'
            new_center_coords, new_all_xyz_coords, dens_v_all, resi_norm = get_new_coords_og(all_coord_info,
                                                                                       r, 
                                                                                       center_coords, 
                                                                                       min_ang, 
...
def place_all_wat(all_coord_info, s,
    ...
    '''
    parser = PDBParser()
    structure = parser.get_structure('PROTEIN', f'{s}.pdb')
    print(structure)
    prot = [atom.get_coord() for atom in structure.get_atoms() if atom.get_parent().get_resname() != 'HOH']
    out_coords_all = []
    norm_all = []
    ...
    for r in resi_list:
        ...
        print(r)
        print(structure)
        result = get_new_all_coords_parallel(all_coord_info, r, min_ang, cont_dict, cutoff_idx, all_xyz_coords, norm_list, structure, use_cutoff)
        if result is not None:
            new_all_xyz_coords, norm_all_tmp = result
        else:
            print('None')
    # Handle the case where function returns None
    # This could be assigning default values, raising an error, etc.
    #new_all_xyz_coords, norm_all_tmp = None, None
        new_all_xyz_coords, norm_all_tmp = get_new_all_coords_parallel(all_coord_info, r, min_ang, cont_dict, cutoff_idx, all_xyz_coords, norm_list, structure, use_cutoff)
        out_coords_all= np.append(out_coords_all,new_all_xyz_coords)
        norm_all = np.append(norm_all,norm_all_tmp) 
...
def build_center_placement_pdb(xyz_coor, pdb_fn, sphere_size_fn):
    ...
    structure = Structure.Structure('New Structure')
    ...
    for i, row in enumerate(list(listy_wat.values())):
        # Create a new Residue
        res_id = (' ', i+1, ' ')
        residue = Residue.Residue(res_id, 'HOH', ' ')

        # Create a new Atom
        coord = np.array([float(row[5]), float(row[6]), float(row[7])])
        atom = Atom.Atom('O', coord, 1.00, 1.00, ' ', 'O', i+1, 'O')
...
def build_density_pdb(xyz_coor, fn, density):
    ...
    for i, (xi, yi, zi, di) in enumerate(zip(np.array(xyz_coor)[:,0][::sample],
        ...
        # Create a new Residue
        res_id = (' ', i+1, ' ')
        residue = Residue(res_id, 'HOH', ' ')

        # Create a new Atom
        coord = np.array([xi, yi, zi])
        atom = Atom('O', coord, 1.00, di, ' ', 'O', i+1, 'O')
...
def get_new_all_coords_parallel(all_coord_info, 
    ...
    dens_v_all = np.array([])
    norm_all = np.array([])
    all_altlocs = set([atom.get_altloc() for atom in res.get_unpacked_list()])
    ...
    for a in all_altlocs:
        poss_idx = [atom for atom in res.get_unpacked_list() if atom.get_altloc() in ['', a]]
        name2idx = {atom.get_name(): idx for idx, atom in enumerate(poss_idx)}
        res_coords = [atom.get_coord() for atom in poss_idx]
        res_names = [atom.get_name() for atom in poss_idx]
        for atom_set_spec, atom_set_gen in DICT4A[res.get_resname()].items(): # go through each set of 4 atoms in this residue
            n_idx=[]
            contin = True
                ...
                for at in atom_set_spec:
                    if len(at)>2: # if part of ring
                        ...
                        if len(ring_idx) == len(at):
                            ring_coords = [res_coords[idx] for idx in ring_idx]
                        ...
                        coords.append(res_coords[name2idx[at]])
                        all_coords.append(res_coords[name2idx[at]])
                dih_val = new_dihedral(np.array(coords))
                bucket=[]
            else:
                for at_s in all_atoms:
                    if at_s in [atom.get_name() for atom in poss_idx]:
                        n_idx.append([atom.get_name() for atom in poss_idx].index(at_s))
                    else:
                        contin = False
                if contin == True:
                    coords = [res_coords[idx] for idx in n_idx]
                    all_coords = coords
                    dih_val = new_dihedral(np.array(coords))
                    bucket=[]
            ...
            if structure != None:
                ...
                for atom in all_atoms:
                   dist = np.linalg.norm([atom.get_coord()] - res_coords[1], axis=1)
                   res_names = [atom.get_name() for atom in all_atoms if np.linalg.norm(atom.get_coord() - res_coords[1]) < 10]
                   res_coords = [atom.get_coord() for atom in all_atoms if np.linalg.norm(atom.get_coord() - res_coords[1]) < 10]
...
def water_RMSD(s, water_new):
    ...
    parser = PDBParser()
    structure = parser.get_structure('PROTEIN', s)
    base_waters = [atom.get_coord() for atom in structure.get_atoms() if atom.get_parent().get_resname() == 'HOH']
    water_yes = 0
    water_no = 0
    all_delta = []
    for w in base_waters:
        delta = np.min(np.linalg.norm(np.array(w) - np.array(water_new.coor), axis=1))
