...
def load_data(pdb):
    ...
    os.chdir('/dors/wankowicz_lab/stephanie/water/water-scripts/4_atom_scripts/new_new_DICT/')
    ...
    norm_val = np.load(f'norm_wat_o_prot_{length}_{pt}{band}.npy',allow_pickle='TRUE').item()
...
def place_all_water(all_coord_info, s, center_coords, min_ang, spread, all_density_vals, cont_dict, cutoff_idx, all_xyz_coords, rel_b_list, q_list, norm_val, pdb):
    ...
    np.save(pdb + '_normalization_wat_o_prot_test_50000.npy', norm_all)
...
def main():
    ...
    os.chdir('/dors/wankowicz_lab/stephanie/water/water-scripts/4_atom_scripts/analysis/')
