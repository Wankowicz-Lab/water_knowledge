...
def make_trim_df(density_all_pdb, kde, cutoff):
    ...
    #print(trim_coor_kde)
    #trim_coor_kde = trim_coor_kde.astype(int).values
    #print(trim_coor_kde)
...
def HDBSCAN(trim_coor_kde):
    ...
    trim_coor_kde['KDE'] = trim_coor_kde['KDE'].astype(float)
    trim_coor_kde['x'] = trim_coor_kde['x'].astype(float)
    trim_coor_kde['y'] = trim_coor_kde['y'].astype(float)
    trim_coor_kde['z'] = trim_coor_kde['z'].astype(float)
    #print(trim_coor_kde)
    #print(trim_coor_kde.info())
...
def build_center_PDB(hoh_coor, file_name, base_pdb): #hoh_coor is a list of tuples of coordinates of water molecules    
    listy_wat = {}
    count=1
    ...
    file = open(f'water_cluster_center_{file_name}.pdb', "w")
    with open(base_pdb, 'r') as base_file:
        base_contents = base_file.read()
        file.write(base_contents)
...
def main():
    ...
    os.chdir("/wynton/group/fraser/swankowicz/water_loc/water-scripts/4_atom_scripts/PDB_files_AA_HS/new_rotamer_lib/")
    kde = np.load(f'{args.pdb}_density_all.npy')
    density_all_pdb = f'{args.pdb}_density_all.pdb' 
    base_pdb = f'{args.pdb}.pdb'
    ...
    build_center_PDB(HOH, args.pdb, base_pdb)   
