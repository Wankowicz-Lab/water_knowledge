...
def structure_based_KDE(coords_all, norm_factor, s, bandwidth):
    ...
    density_all = score_samples(kde, coords_all)
    return density
    ...
    for c in set(s_wat.chain):
        for r in set(s_wat.extract("chain", c, "==").resi):
            if len(np.unique(s_wat.extract(f'chain {c} and resi {r}').altloc)) > 1:
              ...
              n += 1
    s.tofile(f'{pdb_out}_wat_o_prot_norm.pdb')
...
def main():
    ...
    s = PDBParser().get_structure('PDB', args.pdb + '.pdb') 
    ...
    os.chdir(args.d)
    density, density_all = structure_based_KDE(out_coords_all_KDE.reshape(-1,3), norm_all, s, args.band)
